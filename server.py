...
def period_from_month_index(index):
    year, month_index = divmod(index - 1, 12)
    return f"{year:04d}-{month_index + 1:02d}"


def period_months(start_period, end_period):
    start = period_month_index(start_period)
    end = period_month_index(end_period)
...
def build_backtest(payload):
    ...
    periods = [period_from_month_index(index) for index in range(min(month_indexes), max(month_indexes) + 1)] if month_indexes else []
